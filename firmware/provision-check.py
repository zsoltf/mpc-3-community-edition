...
with tempfile.TemporaryDirectory(prefix='mpclearn-provision-') as tmp:
    ...
    stage=root/'data/mpclearn-model.v0_2_2'
    ...
    provision()
    assert selection.read_text()=='/data/mpclearn-model.v0_2_2\n'
    assert stage.stat().st_mode&0o777==0o700 and selection.stat().st_mode&0o777==0o600
    run(['chroot',str(root),'/bin/sh','-c','cd /data/mpclearn-model.v0_2_2 && sha256sum -c session-package.sha256'])
    ...
    (state/'installed').write_text(old_revision);selection.write_text('/data/mpclearn-model.v0_2_2\n')
    ...
    for marker,selected in [('', '/data/mpclearn-model.v0_2_2\n'), ('\n', '/data/mpclearn-model.v0_2_2\n'), ('unknown-version\n','/data/mpclearn-model.v0_2_2\n'), (old_revision,'/data/my-custom-stage\n'), (None,'/data/my-custom-stage\n')]:
    ...
    print('PASS: shipped ARM provisioner fresh install, same-package migration, rerun, disabled-state preservation, r2, r3, r4, r5 and v0.2.0 upgrades with helper replacement, custom-helper refusal, disabled upgrade, interrupted upgrade, settings/project preservation and conflict refusal')
