...
int main(int argc,char **argv){
 ...
 need(!processor_status(argv[1]),"actual client stable CMD31 file readback");fd=open(argv[1],O_RDWR);uint32_t version=25;need(pwrite(fd,&version,4,4)==4,"prior version fixture");close(fd);need(processor_status(argv[1])==1&&processor_stream(argv[1],"1")==1,"CMD25 rejected by status and streaming client");unlink(argv[1]);free(command_state);free(mirror_state);puts("PASS exact CMD31 client file readback and prior CMD25 rejection; native MPC process identity and live stream remain untested");return 0;
}
